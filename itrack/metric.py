...
from itrack.coord import box2corners2d
...
def new_iou3d(box_a, box_b):
    box_a = BBox.array2bbox(box_a)
    box_b = BBox.array2bbox(box_b)
...
def iou3d(box_a, box_b):
    ...
    iou_2d = overlap_area / (reca.area + recb.area - overlap_area)
    ha, hb = box_a["height"], box_b["height"]
    za, zb = box_a["center_z"], box_b["center_z"]
    overlap_height = max(0, min((za + ha / 2) - (zb - hb / 2), (zb + hb / 2) - (za - ha / 2)))
    overlap_volume = overlap_area * overlap_height
    union_volume = box_a["width"] * box_a["length"] * ha + box_b["width"] * box_b["length"] * hb - overlap_volume
    iou_3d = overlap_volume / union_volume

    return iou_2d, iou_3d


def cham_dist(x, y, metric="l2", direction="bi"):
    if direction == "y_to_x":
...
@torch.no_grad()
def compare_meshes_points(pred_meshes, gt_points, num_samples=50000, reduce=True, return_racall=False):
    pred_points, pred_normals = _sample_meshes(pred_meshes, num_samples)
    ...
    if return_racall:
        (
            chamfer_l2,
            recall,
        ) = _compute_sampling_metrics(gt_points, pred_points, return_recall=return_racall)
    else:
        chamfer_l2 = _compute_sampling_metrics(gt_points, pred_points, return_recall=return_racall)

    if reduce:
        # Average each metric over the batch
        chamfer_l2 = chamfer_l2.mean().item()

    if return_racall:
        return chamfer_l2, recall
    else:
        return chamfer_l2
...
def _compute_sampling_metrics(pred_points, gt_points, thres=0.2, return_recall=False):
    lengths_pred = torch.full(
        (pred_points.shape[0],), pred_points.shape[1], dtype=torch.int64, device=pred_points.device
    )
    lengths_gt = torch.full((gt_points.shape[0],), gt_points.shape[1], dtype=torch.int64, device=gt_points.device)
    ...
    chamfer_l2 = pred_to_gt_dists2.mean(dim=1)

    if not return_recall:
        return chamfer_l2
    else:
        recall = torch.sum(pred_to_gt_dists < thres) / pred_to_gt_dists.shape[1]
        return chamfer_l2, recall
